...
unsigned int buildJInstruction(unsigned char opcode,
                               int immediate){
     unsigned int machineCode = (immediate & 0x03FFFFFF);
     machineCode |= ((unsigned int)opcode << 26);
...
unsigned int buildRInstruction(unsigned char opcode,
     ...
     unsigned int machineCode = (function & 0x3F);
     machineCode |= ((unsigned int)shamt << 6);
     machineCode |= ((unsigned int)rd << 11);
     machineCode |= ((unsigned int)rt << 16);
     machineCode |= ((unsigned int)rs << 21);
     machineCode |= ((unsigned int)opcode << 26);
...
void setupInstructionMemory( char* base_memory_address,
    ...
    do {
        ...
        // Answer: 0b000000
            ...
            // Then combine the answer above and finish this part of the code.
            opcode = 0b000000;
            machineCode = buildRInstruction(opcode, instructionStorage[i].rs, instructionStorage[i].rt, instructionStorage[i].rd, 0, 0x20);
            write_dword(base_memory_address, codeOffset+i*4, machineCode);
        }
        ...
        // Answer: 0b000010
            ...
            // Then combine the answer above and finish this part of the code.
            opcode = 0b000010;
            machineCode = buildJInstruction(opcode, instructionStorage[i].address);
            write_dword(base_memory_address, codeOffset+i*4, machineCode);
        }
